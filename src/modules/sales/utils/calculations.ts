 ...
 */
export const calculateSalePriceWithMarkup = (
  costPrice: number,
  markup: number
): number => {
...
export const groupProductsByCategory = (productItems: Sale["productItems"]) => {
  const groups: Record<
    string,
    { count: number; total: number; items: SaleProductItem[] }
  > = {};
...
export const calculateSaleStatistics = (productItems: Sale["productItems"]) => {
  ...
  const mostExpensiveItem = productItems!.reduce<{
 ...
 */
export const getSummaryCalculations = (
  productItems: Sale["productItems"],
  discount: number
) => {
...
export const calculateServicesProfit = (
  serviceItems: SaleServiceItem[]
): number => {
  return serviceItems.reduce((acc, item) => {
    const service = item.service;
    return acc + ((service?.price || 0) - (service?.price || 0));
  }, 0);
};
